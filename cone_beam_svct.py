...
class TigreAxFunction(Function):
    @staticmethod
    def forward(ctx, input_tensor, geometry, angles):
        # Convert PyTorch tensor to numpy ndarray
        input_np = input_tensor.detach().cpu().numpy()
        
        # Perform the tigre.Ax operation
        result_np = tigre.Ax(input_np, geometry, angles)
        ...
        result_tensor = torch.tensor(result_np, device=input_tensor.device)
    ...
    def backward(ctx, grad_output):
        ...
        # Get the geometry and angles for the backward operation
        geometry = ctx.geometry
        angles = ctx.angles
        
        # Convert grad_output to numpy for tigre.At
        grad_output_np = grad_output.cpu().numpy()
        
        # Compute the backward operation using tigre.At
        grad_input_np = tigre.Atb(grad_output_np, geometry, angles)
        
        # Convert the result back to a tensor
        grad_input = torch.tensor(grad_input_np, device=input_tensor.device)
        
        return grad_input, None, None  # None for geometry and angles as they are not tensors


# Define a function for ease of use
def tigre_forward_projection(volume, geo, angles):
    return TigreAxFunction.apply(volume, geo, angles)


def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    first_iter = 0
    gaussians = DGR(dataset.sh_degree)
    scene = CTGeometry(dataset, gaussians, mode='cbct')
    ...
    for iteration in tqdm(range(first_iter, max_iter + 1)):        
        ...
        st = time.time()
        projs = tigre_forward_projection(reconstruct_volume, gaussians.geo, gaussians.angles)
        # print(time.time() - st)
        worldspace_points = gaussians.get_xyz
        radii = gaussians.max_radii2D
        # compute the l2_loss between projs and gaussians.projs
        # l2_loss = torch.nn.MSELoss()
        # loss = l2_loss(projs, gaussians.gt_projs.cuda())
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0 or iteration == 1:
                ...
                print('Iteration:', iteration, 'Gaussian count', gaussians.get_xyz.shape[0], 'Box Size', impact_shape.cpu().numpy().tolist(), 'loss', format(loss.item(), '.4f'),  end=', ')
            ...
            if True:
                ...
                if True:
                    sample_img = reconstruct_volume[reconstruct_volume.shape[0]//2].cpu().numpy()
